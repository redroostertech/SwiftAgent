...
/// Constants for SwiftAgent's Bonjour / DNS-SD advertisement.
///
...
public enum AgentBonjour {
    /// The DNS-SD service type SwiftAgent advertises.
    ///
    ...
    /// Keys used in the Bonjour TXT record SwiftAgent advertises alongside
    /// the service. All values are strings so the record stays
    /// DNS-SD-clean.
    public enum TXTKey {
        /// The SwiftAgent protocol version the server speaks. Matches
